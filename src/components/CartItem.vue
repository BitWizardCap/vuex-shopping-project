...
export default {
    ...
    setup(props) {
        ...
        const changeQuantity = (action = 'add') => {
            if (action == 'add') {
                props.product.quantity = props.product.quantity + 1;
                store.commit("updateCartItem", props.product); // call 'updateCartItem 'action from store
            } else {
                if (props.product.quantity > 1) {
                    props.product.quantity = props.product.quantity - 1;
                    store.commit("updateCartItem", props.product);
                } else {
                    store.commit("removeCartItem", props.product); // call 'removeCartItem' action from store
                }
            }
        }
